...
if __name__ == '__main__':
    ...
    classifier1 = MLPClassifier(5, activation='relu', learning_rate_init=0.001, random_state=0)
    classifier2 = MLPClassifier(10, activation='relu', learning_rate_init=0.001, random_state=0)
    ...
    for i, c in enumerate([classifier1, classifier2, classifier3]):
        val_pred = c.predict(val_x)
        val_acc = 0
        for pred, true in zip(val_pred, val_y):
            if pred == true:
                val_acc +=1
    ...
    print(f'Klasifikatorot {final_classifier} ima najdobra tochnost od {max_acc}')
    #znaci tia so 100 nevroni u skrieniot sloj
