...
if __name__ == '__main__':
    ...
    new_train_x = list()
    for row in train_x:
        sample = [ row[ i ] for i in range(len(row)) if i != index_col]
    ...
    classifier1 = RandomForestClassifier(n_estimators=num_trees, criterion=kriterium, random_state=0)
    classifier1.fit(new_train_x, train_y)

    acc = 0
    predicted = classifier1.predict(new_test_x)
    for pred , true in zip(predicted, test_y):
        if pred == true:
            acc +=1
    print(f'Accuracy: {acc/len(test_y)}')

    classifier2 = RandomForestClassifier(n_estimators=num_trees, criterion=kriterium, random_state=0)
    classifier2.fit(train_x, train_y)

    output1 = classifier2.predict([entry])
    output2 = classifier2.predict_proba([entry])
    print(output1[0])
    print(output2)
